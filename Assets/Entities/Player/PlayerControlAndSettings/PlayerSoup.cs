...
public class PlayerSoup : MonoBehaviour
{
    ...
    // Calculates Spoon Stats, Same as Fill Spoon, but doesn't actually make the spoon
    public Spoon CalcPossibleSpoonStats(List<FlavorIngredient> flavor, List<AbilityIngredient> ability)
    {
        if (ability == null || ability.Count == 0)
        {
            Debug.Log("CalcSpoonStats: Ability list is empty, spoon can't be made!");
        }
        
        // Make a New Spoon Unrelated To The Player Spoons
        Spoon spoon = new();

        // make spoon empty
        spoon.MakeEmpty();

        // set max usage to zero
        spoon.maxUsage = 0;

        // Make a new list for the pot
        List<(string, int)> pot = new List<(string, int)>();
    ...
    public string SoupInfo(List<FlavorIngredient> flavor, List<AbilityIngredient> ability){ 
        ...
        Spoon spoon = new();
        spoon = CalcPossibleSpoonStats(flavor, ability);
