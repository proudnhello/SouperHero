...
public class CookingFlavorIcon : MonoBehaviour, ITooltipSource
{
    ...
    void SetIcon(FlavorIconInfo flavorInfo, float amount)
    {
        ...
        int intAmount = Mathf.Clamp(Mathf.FloorToInt(amount), 0, 5);
        ...
        float sizeX = Mathf.Abs(transform.position.x - tickObjects[intAmount - 1].transform.position.x) * colliderDistanceMultiplier;
        boxCollider2D.size = new Vector2(sizeX, boxCollider2D.size.y);
        boxCollider2D.offset = new Vector2(sizeX / 2, 0);
    }
